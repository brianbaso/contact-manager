...
// Delete all contacts
app.delete("/contacts", async (request, response) => {
  try {
    // create a snapshot of the firestore db
    const contactsQuerySnapshot = await db.collection("contacts").get();
...
app.post("/message", async (request, response) => {
  try {
    ...
    // Create a new collection in the firestore db if needed, otherwise add to
    // the existing colleciton
    const contactRef = await db.collection("messages").add(data);
    const contact = await contactRef.get();
...
app.get("/messages", async (request, response) => {
  try {
    // create a snapshot of the firestore db
    const messagesQuerySnapshot = await db.collection("messages").get();

    // create an array for the messages to go into
    const messages: any[] = [];
